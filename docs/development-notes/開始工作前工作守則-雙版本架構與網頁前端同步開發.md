...
9. 備份完成後要確認壓縮檔真的存在，且至少包含 `package.json`、`docs/`、`scripts/`、`.github/`、`backup-manifest.json` 與 `RESTORE-INSTRUCTIONS.md`。
...
$env:PMC_WEB_PORT="3106"
npm run server
```

也可以用參數方式啟動 server-only 模式：

```powershell
npm run server -- --host 0.0.0.0 --port 3106
```

桌面版 `npm start` 會預設啟動內建 server 在 `3105`；若需調整，使用：

```powershell
$env:PMC_DESKTOP_SERVER_PORT="3105"
npm start
```
...
不再預設放到專案資料夾內的 `backups/`。如果只是臨時測試或要改放其他位置，需明確指定 `-DestinationRoot`。
